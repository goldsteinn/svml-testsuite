...
# define I_lsort(base, nelem, T, lname, ...)                                   \
        {                                                                      \
            int lname(void const * vlhs, void const * vrhs) {                  \
                T lhs;                                                         \
                T rhs;                                                         \
                lhs = (T)vlhs;                                                 \
                rhs = (T)vrhs;                                                 \
            ...
            qsort(base, nelem, sizeof(base[0]), lname);                        \
        }

# define lsort(base, nelem, ...)                                               \
        I_lsort(base, nelem, get_type((base)[0]) const *,                      \
                CAT(lname, I_COUNTER), __VA_ARGS__)
