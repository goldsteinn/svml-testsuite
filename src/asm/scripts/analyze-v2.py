...
import sys
import copy
from common_util import (strs, is_int, vprint, set_verbosity, tern, vvvprint,
...
def is_pseudo_class(tname):
    return tname in {"flag", "barrier"}


def is_standard_reg_class(tname):
    return tname in {"vec", "gpr", "mask"}
...
def get_op_info(op_def):
    ...
    if op_type in {Op_Types.AGEN, Op_Types.IMM, Op_Types.LABEL}:
        ...
        if is_int(op_size):
            op_size = int(op_size)
            assert op_size in {4, 8, 16, 32, 64}
        else:
            global G_all_regs
...
class Sub_Op_Info():
    ...
    def is_reg(self):
        ...
        ) or self.is_flag() or self.is_ip() or self.is_barrier()

    def is_standard_reg(self):
        return self.is_vec() or self.is_gpr() or self.is_mask()
    ...
    def is_barrier(self):
        return self.op_type == Op_Types.BARRIER
...
class Op_Possibilities():
    ...
    def reg_valid(self):
        return self.check([
            ...
            Op_Types.FLAG, Op_Types.BARRIER
        ])
...
class Op_Info():
    ...
    def mustbe_barrier(self):
        return self.barrier_info() == 1
    ...
    def mustbe_results(self):
        ...
        if self.mustbe_barrier():
            assert self.mustbe_reg()
            res.add(Op_Types.BARRIER)
        if self.mustbe_ip():
            assert self.mustbe_reg()
            res.add(Op_Types.IP)
        if self.mustbe_imm():
...
class Reg_Proto():
    ...
    def is_pseudo(self):
        return is_pseudo_class(ssa_name_to_type_name(self.ssa_name))
...
def ssa_name_to_type_name(ssa_name):
    ...
    assert len(tname) in {2, 3, 4, len("barrier")}
    return tname
...
class Allocation():
    ...
    def __init__(self, reg_or_stack):
        assert reg_or_stack is None or isinstance(
            reg_or_stack, Reg_Info) or isinstance(reg_or_stack, StackSpot)
    ...
    def x86_name(self, opt):
        if self.alloc is None:
            return ""
        elif isinstance(self.alloc, Reg_Info):
            global G_all_regs
...
class Insn():
    ...
    def __init__(self, iid, lbl, lc, pieces):
        vvprint(str(pieces))
        global G_all_insns
        global G_all_regs
        global G_reg_map
        ...
        self.recursive_uses = None
        self.uses = None
        self.defs = None
        self.xdefs = set()
        self.needed = False
        self.is_ret = False
        self.implicit_uses = set()
        self.implicit_defs = set()
        self.last_uses = set()
        ...
        self.jmp = self.insn_info.jmp
        self.cond = self.insn_info.cond
        self.is_true_ret = self.insn_info.is_true_return
        ...
        for op in self.ops:
            if op.op_info.is_write():
                dsts += 1
        assert dsts == 1 or self.insn_info.is_true_return, self.name

        if not self.insn_info.is_true_return:
            assert self.insn_info.num_ops() > 0
            assert self.ops[-1].op_info.is_write(), self.name

    def add_implicit_use(self, ssa_name):
        assert len(self.implicit_uses) == 0
        self.implicit_uses.add(ssa_name)
            ...
            if self.ops[i].op_info.is_read():
                ...
                if elim[1] in uses:
                    if isinstance(self.ops[i].opt, Reg):
                        assert len(uses) == 1
                        assert is_standard_reg_class(
                            ssa_name_to_type_name(elim[0]))
                        assert is_standard_reg_class(
                            ssa_name_to_type_name(
                                self.ops[i].opt.proto_in.ssa_name))
    ...
    def collect_recursive_uses(self, insns):
        global G_all_labels
        assert self.recursive_uses is None
        assert self.uses is not None
        assert self.defs is not None

        self.recursive_uses = set()
        if not self.jmp:
            return
        assert self.insn_info.num_ops() == 4
        assert isinstance(self.ops[1].opt, Label)
        lbl = self.ops[1].opt.lbl

        assert lbl != self.lbl
        assert lbl in G_all_labels

        target_defs = set()
        target_uses = set()
        target_lbls = G_all_labels[lbl] | {lbl}
        for i in range(0, len(insns)):
            if insns[i].lbl in target_lbls:
                target_defs |= insns[i].defs
                target_uses |= insns[i].uses
        missing = target_uses - target_defs
        for maybe_missing in missing:
            if is_pseudo_class(ssa_name_to_type_name(maybe_missing)):
                continue
            self.recursive_uses.add(maybe_missing)

        self.uses |= self.recursive_uses

    def collect_use_defs(self):
        assert self.uses is None
        assert self.defs is None

        self.uses = copy.deepcopy(self.implicit_uses)
        self.defs = copy.deepcopy(self.implicit_defs)
        for op in self.ops:
...
def split_lines(f):
    ...
    for line in open(f):
        ...
        if line.count("(") == 1 and line.count(")") == 1:
            lowline = line.lower().split()
            hit = False
            if (lowline[0].startswith("l(") or
                (len(lowline) > 1 and lowline[0] == "l"
                 and lowline[1][0] == "(")) and line.endswith("):"):
                hit = True
                if cur_block is not None:
                    cur_block.insn_lines = copy.deepcopy(lines)
                    cur_block.comments = copy.deepcopy(comments)
                    code_blocks.append(copy.deepcopy(cur_block))
            if (lowline[0].startswith("entry(") or
                (len(lowline) > 1 and lowline[0] == "entry" and lowline[1][0]
            ...
            if hit:
                ...
                cur_block = CodeBlock(lc, line)
                lines = []
                comments = []
                continue

        if cur_block is None:
            cur_block = CodeBlock(lc, None)
    ...
    for i in range(0, len(insns)):
        ...
        if good % 2 == 0:
            if insns[-(i + 1)].name == "ret":
                ...
                vprint("New Needed: {}".format(insns[-(i + 1)].to_str()))
        else:
...
def collect_DFG_deps(insns, insn, all_defs, all_uses, completed):
    ...
    for dep in deps:
        all_deps = all_deps | collect_DFG_deps(insns, dep, all_defs, all_uses,
                                               completed)
    return all_deps


def build_DFG(insns, strat):
    vprint("Building DFG")
    for insn in insns:
        for op in insn.ops:
            if isinstance(op.opt, Reg):
    ...
    for insn in insns:
        insn.collect_recursive_uses(insns)
    ...
    for iid in end_goal:
        insn = insns[iid]
        all_deps = all_deps | collect_DFG_deps(insns, insn, all_defs, all_uses,
                                               set())
    ...
    max_use = {}
    assert len(all_pops) == len(all_deps), "{}".format(all_deps - all_pops)
    ...
    if True:
        ...
        def strh_order(insn):
            ...
            if insn.iid not in all_pops:
                for use in insn.uses:
                    assert use not in cur_defs
                for dep in insn.deps:
                    assert dep in all_pops
                all_pops.add(insn.iid)
...
def new_move_insn(insn):
    ...
    insn_out.implicit_uses = insn.implicit_uses
...
def reinsert_moves(insns):
    ...
    for i in range(0, len(insns)):
        for use in insns[i].uses:
            if true_last_uses[use] == i:
                assert (not insns[i].jmp) or (not is_standard_reg_class(
                    ssa_name_to_type_name(use))) or (
                        insns[i].ops[2].opt.proto_in.rtype == Op_Types.GPR)
    ...
    while changed:
        ...
        for i in range(0, len(insns)):
            insns[i].iid = i
            insns[i].reset_dfg_info()
            insns[i].collect_use_defs()
        for i in range(0, len(insns)):
            insns[i].collect_recursive_uses(insns)
        for i in range(0, len(insns)):
            for use in insns[i].uses:
                true_last_uses[use] = i
        ...
        for i in range(0, len(insns)):
            for op in insns[i].ops:
                if len(moves) == 0:
                    break
                if (isinstance(op.opt, Reg) or isinstance(op.opt, Agen)):
                    if isinstance(op.opt, Reg):
                        if op.opt.proto_out is not None:
                            assert op.opt.proto_out.ssa_name not in moves
                        if not op.op_info.is_read():
                            continue
            ...
            if len(moves) == 0:
                ...
                if lu is not None:
                    ...
                    if i != true_last_uses[lu]:
                        vvvprint("Last Use:({}) {} ({} != {})\n\t->{}".format(
                            insns[i].to_str(), lu, true_last_uses[lu], i,
                            insns[true_last_uses[lu]].to_str()))
                        ...
                        if not did_commute:
                            ...
                            assert not is_pseudo_class(
                                ssa_name_to_type_name(mov_src))
                            assert not is_pseudo_class(
                                ssa_name_to_type_name(mov_dst))
                            ...
                            assert is_standard_reg_class(
                                ssa_name_to_type_name(mov_dst))
                            global G_reg_map
                            vvvprint("Handling Move: {}/{}".format(
                                moves, G_reg_map.reg_cnt.rcnts[Op_Types.VEC]))
...
def build_liveness(insns, misused):
    ...
    for i in range(0, len(fixed_regs)):
        ...
        if is_pseudo_class(ssa_name_to_type_name(fixed_ssas[i])):
            continue
    ...
    for i in range(0, len(insns)):
        vvvprint("{} -- {}".format(insns[i].to_str().ljust(40),
                                   ", ".join(insns[i].uses)))
...
def reg_alloc(insns):
        ...
        for op in insn.ops:
            if isinstance(op.opt, Reg):
                if op.opt.proto_in is not None:
                    ...
                    if op.opt.proto_in.fixed and not op.opt.proto_in.is_pseudo(
                    ):
                ...
                if op.opt.proto_out is not None:
                    ...
                    if op.opt.proto_out.fixed and not op.opt.proto_in.is_pseudo(
                    ):
    ...
    for tname in fixed_colors:
        for color in fixed_colors[tname]:
            ...
            assert reg in avail_regs[tname], "{} not in {}".format(
                reg, avail_regs[tname])
    ...
    for tname in color_counts:
        ...
        for color in sorted(color_counts[tname],
            ...
            alloc = None
            if is_pseudo_class(tname):
                pass
            elif usable_regs is None:
            ...
            assert alloc is None or isinstance(alloc, StackSpot) or isinstance(
                alloc, Reg_Info)
            allocs.setdefault(tname, {})
            assert color not in allocs[tname]
    ...
    for sz in sorted(stacks):
        ...
        for tname in allocs:
            for c in allocs[tname]:
                if allocs[tname][c] is None:
                    pass
                elif isinstance(allocs[tname][c], StackSpot):
...
def optimize(blocks):
    insns = []
    for block in blocks:
        insns += copy.deepcopy(block.insns)
    ...
    for strat in range(0, strat_cnt):
        for i in range(0, iter_cnt):
            ...
            insns, init_regs = build_DFG(insns, strat)
        ...
        if not only_one:
            ...
            for i in range(0, iter_cnt):
                ...
                for j in range(0, i + 1):
                    tmp_insns, init_regs = build_DFG(tmp_insns, strat)
...
class CodeBlock():
    ...
    def initialize(self):
        global G_all_labels
        assert self.lbl not in G_all_labels
        G_all_labels[self.lbl] = set()
        assert len(self.insn_lines) > 0, self.lbl
        assert len(self.insns) == 0

        i = 0
        insns = []
        for insn_pieces in self.insn_lines:
            insn = Insn(i, self.lbl_name(), insn_pieces[0], insn_pieces[1])
            insns.append(insn)
            i += 1
        self.insns = set_return(insns)

    def block_deps(self):
        global G_all_labels
        for insn in self.insns:
            if insn.jmp:
                assert insn.insn_info.num_ops() == 4
                assert isinstance(insn.ops[1].opt, Label)
                dst = insn.ops[1].opt.lbl
                assert dst != self.lbl
                assert dst in G_all_labels, "'{}' not in '{}'".format(
                    dst, G_all_labels)
                G_all_labels[self.lbl].add(dst)

    def barrier_ssa_out(self):
        ret_idx = None
        for i in range(0, len(self.insns)):
            if self.insns[i].is_true_ret:
                assert ret_idx is None
                ret_idx = i
        if ret_idx is None:
            return None
        barrier = self.insns[ret_idx].get_dst()
        assert isinstance(barrier.opt, Reg)
        assert barrier.op_info.is_barrier()
        assert barrier.opt.proto_out is not None
        assert barrier.op_info.is_read()
        assert barrier.op_info.is_write()
        return barrier.opt.proto_out.ssa_name

    def real_label(self):
        return self.lbl is not None
...
def initialize_blocks(blocks):
    ...
    while changed:
        changed = False
        for lbl in G_all_labels:
            cpy = copy.deepcopy(G_all_labels[lbl])
            for sub_lbl in cpy:
                assert sub_lbl != lbl
                for new_lbl in G_all_labels[sub_lbl]:
                    assert new_lbl != lbl
                    changed |= (new_lbl not in G_all_labels[lbl])
                    G_all_labels[lbl].add(new_lbl)
...
setup(sys.argv[2])
blocks = split_lines(sys.argv[3])
blocks = initialize_blocks(blocks)

for i in range(0, len(blocks)):
    barrier_ssa = blocks[i].barrier_ssa_out()
    if barrier_ssa is None:
        assert i + 1 == len(blocks)
        break
    assert i + 1 < len(blocks)
    for insn in blocks[i + 1].insns:
        insn.add_implicit_use(barrier_ssa)

insns = optimize(blocks)
for insn in insns:
    insn.p_x86out()
